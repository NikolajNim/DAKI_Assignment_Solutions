...
dataset = pd.read_csv('HeartDiseaseHealthIndicatorsDataset/heart_disease_health_indicators_BRFSS2015.csv')
...
'### Preprocess data ###'
## balance data
dataset = balance_data(dataset, rows_in_each_class=23893) # there are 229787 entries with label 1 and 23893 entries with label 0 --> max value in balance_data funtion must be 23893 to balance data
...
# change amount of training instances based on previous split
X_train_reduced, _, y_train_reduced, _ = train_test_split(X_train, y_train, test_size=0.40, random_state=42)
...
X_test_minmax = minmax_scalar.fit_transform(X_test) # transform data
